...
class MtcnnDetector(object):
    ...
    def detect_rnet(self, im, dets):
        ...
        cls_map, reg, landmark = self.rnet_detector(feed_imgs)
        ...
        for i in range(2*config.NUM_LANDMARKS):
            if i % 2 == 0:
                landmarks_list.append(align_landmark_topx + keep_landmark[:, i]*bw)
            else:
                landmarks_list.append(align_landmark_topy + keep_landmark[:, i]*bh)
        
        landmark = np.vstack(landmarks_list)
        
        landmark_align = landmark.T
        # print(landmark_align)
    ...
    def detect_face(self, img):
        ...
        if self.rnet_detector:
            boxes_align, landmark_align = self.detect_rnet(img, boxes_align)
            if boxes_align is None:
                return np.array([]), np.array([])
