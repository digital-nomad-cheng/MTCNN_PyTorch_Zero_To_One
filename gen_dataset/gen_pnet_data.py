...
def gen_pnet_data(data_dir, anno_file, prefix):
    ...
    image_size = config.PNET_SIZE
    # data save dir
    neg_save_dir = os.path.join(data_dir, "pnet", str(config.PNET_SIZE), "negative")
    pos_save_dir = os.path.join(data_dir, "pnet", str(config.PNET_SIZE), "positive")
    part_save_dir = os.path.join(data_dir, "pnet", str(config.PNET_SIZE), "part")
    ...
    for annotation in annotations:
        ...
        neg_num = 0
        # 50 negative samples for each image
        while neg_num < 20:
            size = npr.randint(image_size, min(width, height) / 2) # 12 is used for keep w, h ratio
        ...
        for box in boxes:
            ...
            if max(w, h) < 40 or x1 < 0 or y1 < 0 or min(w, h) < 10:
                continue
