...
def gen_data(anno_file, data_dir, prefix):
    size = config.RNET_SIZE

    landmark_imgs_save_dir = os.path.join(data_dir, 'rnet', str(config.RNET_SIZE), "landmark")
    ...
    for annotation in annotations:
        ...
        gt_box = [gt_box[0], gt_box[1], gt_box[2], gt_box[3]]  # [left, top, right, bottom]
        ...
        for i in range(20):
            ...
            offset = [0] * (2*config.NUM_LANDMARKS)
            ...
            for k in range(config.NUM_LANDMARKS):
                offset[2*k] = float(landmark[2*k] - nx1) / float(bbox_size)
                offset[2*k+1] = float(landmark[2*k+1] - ny1) / float(bbox_size)
            ...
            if iou > 0.65:
                ...
                f.write(save_file + ' -2 %.2f %.2f %.2f %.2f ' %
                    (offset_x1, offset_y1, offset_x2, offset_y2))
                for i in range(2*config.NUM_LANDMARKS):
                    f.write('%.2f ' % offset[i])
                f.write('\n')
                l_idx += 1
...
def parse_args():
    ...
    parser.add_argument('--anno_file', dest='annotation_file', help='landmark dataset original annotation file',
                        default=os.path.join(config.ANNO_STORE_DIR, config.BASE_LANDMARK_ANNO_FILE), type=str)
    parser.add_argument('--prefix_path', dest='prefix_path', help='annotation file image prefix root path',
                        default=config.BASE_LANDMARK_PATH, type=str)
