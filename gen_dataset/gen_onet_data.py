...
def get_onet_sample_data(data_dir, anno_file, det_boxs_file, prefix):
    ...
    image_size = config.ONET_SIZE
    # data save dir
    neg_save_dir = os.path.join(data_dir, "onet", str(config.ONET_SIZE), "negative")
    pos_save_dir = os.path.join(data_dir, "onet", str(config.ONET_SIZE), "positive")
    part_save_dir = os.path.join(data_dir, "onet", str(config.ONET_SIZE), "part")
    ...
    part_save_file = os.path.join(
        config.ANNO_STORE_DIR, config.ONET_PART_ANNO_FILENAME)
...
def parse_args():
    ...
    parser.add_argument('--anno_file', dest='annotation_file', help='wider face original annotation file',
                        default=os.path.join(config.ANNO_STORE_DIR, config.BASE_DATA_ANNO_FILE), type=str)
    ...
    parser.add_argument('--rmodel_file', dest='rnet_model_file', help='RNet model file path',
                         default='./results/rnet/check_point/{0}_landmarks_model_050.pth'.format(config.NUM_LANDMARKS), type=str)
    # parser.add_argument('--pmodel_file', dest='pnet_model_file', help='PNet model file path',
    #                     default='./results/pnet/save/pnet.pth', type=str)
    # parser.add_argument('--rmodel_file', dest='rnet_model_file', help='RNet model file path',
    #                     default='./results/rnet/save/rnet.pth', type=str)
...
if __name__ == '__main__':
    ...
    gen_onet_data(args.traindata_store, args.annotation_file, args.pnet_model_file, 
        args.rnet_model_file, args.prefix_path, use_cuda=False)
    
    # get_onet_sample_data(args.traindata_store, args.annotation_file, 
    #     det_boxs_file='./data/pnet_rnet_detections_1569510995.pkl',
