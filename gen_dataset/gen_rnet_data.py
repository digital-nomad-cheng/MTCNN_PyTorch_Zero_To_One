...
from tools.image_reader import ImageLoader
...
def gen_rnet_data(data_dir, anno_file, pnet_model_file, prefix_path='', use_cuda=True, visualize=False):
    ...
    image_reader = ImageLoader(imdb, 1, False)
    ...
    for databatch in image_reader:
        ...
        if boxes_align is None:
            ...
            continue
        if visualize:
            vis.vis_face(im, boxes_align)
            vis.vis_face(im, boxes)
        t1 = time.time() - t
...
def get_rnet_sample_data(data_dir, anno_file, det_boxes_file, prefix_path):
    neg_save_dir = os.path.join(data_dir, 'rnet', str(config.RNET_SIZE), "negative")
    pos_save_dir = os.path.join(data_dir, 'rnet', str(config.RNET_SIZE), "positive")
    part_save_dir = os.path.join(data_dir, 'rnet', str(config.RNET_SIZE), "part")
    ...
    part_save_file = os.path.join(
        config.ANNO_STORE_DIR, config.RNET_PART_ANNO_FILENAME)
    ...
    for im_idx, dets, gts in zip(im_idx_list, det_boxes, gt_boxes_list):
        ...
        for box in dets:
            ...
            if np.max(Iou) < 0.3:
                ...
                cur_n_idx += 1
                if cur_n_idx <= 30: # 30 negative samples per image at most
                ...
                if np.max(Iou) >= 0.65:
                    ...
                    cv2.imwrite(save_file, resized_im)
                    d_idx += 1
        print("negative:{0}, positive: {1}, part: {2}".format(n_idx, p_idx, d_idx))
    f1.close()
    f2.close()
    f3.close()
...
def parse_args():
    ...
    parser.add_argument('--anno_file', dest='annotation_file', help='wider face original annotation file', default=os.path.join(config.ANNO_STORE_DIR, config.BASE_DATA_ANNO_FILE), type=str)
    parser.add_argument('--pmodel_file', dest='pnet_model_file', help='PNet model file path',
                        default='./results/pnet/check_point/{0}_landmarks_model_050.pth'.format(config.NUM_LANDMARKS), type=str)
    ...
    parser.add_argument('--prefix_path', dest='prefix_path', help='annotation file image prefix root path',
                        default=config.BASE_DATA_PATH, type=str)
...
if __name__ == '__main__':
    ...
    gen_rnet_data(args.traindata_store, args.annotation_file,
        args.pnet_model_file, args.prefix_path, args.use_cuda, visualize=False)
    # get_rnet_sample_data(args.traindata_store, args.annotation_file, 
    #    os.path.join(config.TRAIN_DATA_DIR, 'pnet_detections_1568969866.pkl'),
